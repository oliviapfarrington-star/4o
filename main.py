...
# Keeping this at 30 prevents hitting the 30k TPM rate limit on lower-tier plans.
...
async def get_completion(
    ...
    If *image_blocks* is provided, they are appended to the **last** user
    message as image_url content blocks so the model can see them.
    """
    ...
    # Attach images to the final user message so the model can see them
    if image_blocks and messages and messages[-1]["role"] == "user":
        last_text = messages[-1]["content"]
        content_blocks: list[dict] = [{"type": "text", "text": last_text}]
...
async def on_message(message: discord.Message) -> None:
    ...
    for att in message.attachments:
        ...
        if ext in IMAGE_EXTS:
            # Download and base64-encode so the image works even if the URL expires
            try:
                ...
                attachment_notes.append(f"[User attached image: {att.filename}]")
            except Exception:
                attachment_notes.append(f"[User attached image: {att.filename} — failed to download]")
        elif ext in AUDIO_EXTS:
            attachment_notes.append(
                f"[User attached audio file: {att.filename} "
                f"({att.size / 1024:.0f} KB) — URL: {att.url}]"
            )
        elif ext in VIDEO_EXTS:
            attachment_notes.append(
                f"[User attached video file: {att.filename} "
                f"({att.size / (1024 * 1024):.1f} MB) — URL: {att.url}]"
            )
        else:
            attachment_notes.append(
                f"[User attached file: {att.filename} ({att.size / 1024:.0f} KB)]"
            )

    # Combine text and attachment descriptions into what gets stored
    parts = []
    if user_text:
    ...
    # Save the user's message (text + attachment descriptions)
    await save_message(user_id, channel_id, "user", combined_text)

    # Build conversation from history
    conversation = await fetch_history(user_id, channel_id)

    # Get the AI reply — pass image URLs for the current message only
    async with message.channel.typing():
        try:
            reply = await get_completion(conversation, image_blocks=image_data or None)
        except Exception as e:
            error_name = type(e).__name__
            print(f"[ERROR] {error_name}: {e}")
            await message.reply(
                f"⚠️ Something went wrong: **{error_name}**\n"
                f"Try `!clear` if the conversation history is too long.",
